#!/bin/bash
pwd=123
echo $pwd | sudo -S apt install intltool itstool
echo $pwd | sudo -S apt install python-dev 
...
echo $pwd | sudo -S apt-get -y install ack-grep tmux ntp ntpdate
echo $pwd | sudo -S apt-get -y install nginx-full
echo $pwd | sudo -S apt-get -y install mysql-server-5.7 mysql-client-5.7 libmysqld-dev mysql-workbench
